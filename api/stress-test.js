...
function buildHtml() {
  ...
  html = replaceOnce(html,'下のボタンでメールアプリを開くと、入力した連絡先と診断結果が本文に入った状態になります。内容をご確認のうえ送信してください。','下のボタンからブラウザ上でそのまま相談内容を送信できます。メールアプリやGmailは必要ありません。','handoff_hint');
  html = replaceOnce(html,'メールは送信前にご自身で内容を確認できます。入力内容・診断結果は、相談対応のために使用します。','入力内容・診断結果は相談対応のために使用します。送信完了後、後日担当者よりメールでご連絡します。','privacy_note');
  html = replaceOnce(html,"$('#handoffGo').textContent=life?'無料相談メールを作成する →':'物件相談メールを作成する →';","$('#handoffGo').textContent=life?'無料相談を送信する →':'物件相談を送信する →';",'handoff_button_copy');
  html = replaceOnce(html,"function openMailToSakai(){if(!handoffMode||!validateIntake())return;const subject=mailSubject(handoffMode),body=summaryText(handoffMode);$('#handoffSummary').textContent=body;window.location.href=`mailto:${SAKAI_EMAIL}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`}","async function submitConsultation(){if(!handoffMode||!validateIntake())return;const c=contactValues(),body=summaryText(handoffMode),btn=$('#handoffGo');$('#handoffSummary').textContent=body;$('#copyState').textContent='送信中です…';btn.disabled=true;try{const r=await fetch('/api/consultation',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:handoffMode,name:c.name,email:c.email,phone:c.phone,summary:body})});const data=await r.json().catch(()=>({}));if(!r.ok||!data.ok)throw new Error(data.error||'submit_failed');$('#copyState').textContent='送信しました。後日担当者よりメールでご連絡します。';btn.textContent='送信済み';btn.disabled=true;}catch(e){console.error(e);$('#copyState').textContent=e.message==='service_not_configured'?'現在、相談受付の送信設定を確認中です。恐れ入りますが、相談内容をコピーして担当者へお送りください。':'送信できませんでした。通信環境をご確認のうえ、もう一度お試しください。';btn.disabled=false;}}",'submit_function');
  html = replaceOnce(html,"$('#handoffGo').onclick=openMailToSakai;","$('#handoffGo').onclick=submitConsultation;",'submit_binding');

  html = replaceOnce(
    html,
    '.cta{background:linear-gradient(135deg,#151515,#282828);color:#fff;border-radius:19px;padding:20px;margin:18px 0}',
    '.lifeplanPreview{border:1px solid #dfd3b7;border-radius:20px;padding:18px;margin:24px 0 14px;background:linear-gradient(180deg,#fffdf8,#fbf6e9)}.lifeplanPreview .lpEyebrow{font-size:11px;font-weight:900;letter-spacing:.11em;color:#9b7426}.lifeplanPreview h3{font-size:20px;margin:7px 0 8px}.lifeplanPreview .lpLead{font-size:12.5px;line-height:1.75;color:#4c463d;margin:0 0 13px}.lpScope{background:#fff;border:1px solid #e1d8c8;border-radius:13px;padding:12px 13px;margin:12px 0}.lpScope b{display:block;font-size:12px;margin-bottom:6px}.lpScope p{font-size:11.5px;line-height:1.7;color:#514b42;margin:0}.lpScope strong{color:#8f691f}.lpQuestions{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin:12px 0}.lpQuestion{background:#fff;border:1px solid #e7dfcf;border-radius:11px;padding:8px 9px;font-size:10.5px;line-height:1.45;color:#595247}.lpQuestion b{display:block;color:#9b7426;font-size:9.5px;margin-bottom:2px}.lpQuestion strong{display:block;color:#2f2b25;font-size:11.5px;margin-bottom:1px}.lpChart{background:#fff;border:1px solid #e0d8ca;border-radius:14px;padding:11px;margin-top:13px}.lpChartHead{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:5px}.lpChartHead b{font-size:12px}.lpChartHead span{font-size:9.5px;color:#7b7469;background:#f4f0e7;border-radius:999px;padding:4px 7px}.lpCompareIntro{font-size:10.5px;line-height:1.55;color:#5e574d;margin:0 0 8px}.lpLegend{display:flex;flex-wrap:wrap;gap:7px 12px;margin:0 0 7px}.lpLegendItem{display:flex;align-items:center;gap:5px;font-size:9.5px;color:#615a50}.lpSwatch{display:inline-block;width:18px;height:3px;border-radius:99px;background:#b88a2e}.lpSwatch.alt{background:repeating-linear-gradient(90deg,#6f6b63 0 6px,transparent 6px 9px)}.lpChart svg{display:block;width:100%;height:auto}.lpChartNote{font-size:10px;line-height:1.6;color:#797267;margin:8px 1px 0}.lpAllocation{margin:13px 0 0;background:#fff;border:1px solid #e0d8ca;border-radius:14px;padding:12px}.lpAllocation h4{font-size:12.5px;margin:0 0 4px}.lpAllocationLead{font-size:10.5px;line-height:1.55;color:#5e574d;margin:0 0 9px}.lpAllocationGrid{display:grid;grid-template-columns:repeat(3,1fr);gap:7px}.lpAllocationCard{border:1px solid #e7dfcf;border-radius:11px;padding:9px;background:#fffdf9;min-width:0}.lpAllocationCard b{display:block;font-size:10.5px;color:#2f2b25;margin-bottom:3px}.lpAllocationCard .lpAllocValue{font-size:12px;font-weight:900;color:#9b7426;margin-bottom:3px}.lpAllocationCard p{font-size:9.5px;line-height:1.5;color:#696156;margin:0}.lpAllocationNote{font-size:9.5px;line-height:1.55;color:#7a7369;margin:8px 1px 0}.lpBridge{font-size:12px;line-height:1.75;color:#3f3a33;margin:12px 0 0;font-weight:700}@media(max-width:640px){.lpAllocationGrid{grid-template-columns:1fr}.lpAllocationCard{display:grid;grid-template-columns:95px 1fr;column-gap:8px;align-items:start}.lpAllocationCard b{grid-column:1}.lpAllocationCard .lpAllocValue{grid-column:1}.lpAllocationCard p{grid-column:2;grid-row:1/3}}.cta{background:linear-gradient(135deg,#151515,#282828);color:#fff;border-radius:19px;padding:20px;margin:14px 0 18px}',
  ...
  const lifeplanPreview = `
      <div class="lifeplanPreview">
        ...
        <div class="lpChart" aria-label="同じ年収でも暮らし方によって金融資産の推移が変わることを示すライフプランのイメージ">
          ...
          <p class="lpCompareIntro">同じ年収・同じ住宅価格でも、教育・車・旅行などへのお金のかけ方が違うだけで、将来の金融資産は大きく変わることがあります。</p>
          ...
          <p class="lpChartNote">※上の2本は違いをイメージするための参考例で、実際の結果や将来を予測するものではありません。金融資産が一時的にマイナスになる場合も、住宅予算・教育方針・働き方・資産運用などを調整しながら確認します。</p>
        </div>
        <div class="lpAllocation" aria-label="住宅に使わなかった余白の配分先による違いのイメージ">
          <h4>同じ「1,000万円の余白」でも、使い方で将来は変わります</h4>
          <p class="lpAllocationLead">住宅予算を抑えたこと自体がゴールではありません。残したお金をどう持つか、どう使うかまで含めて比較します。</p>
          <div class="lpAllocationGrid">
            <div class="lpAllocationCard"><b>現金で残す</b><div class="lpAllocValue">1,000万円 → 1,000万円</div><p>元本をそのまま持つ例。安心感はありますが、ここでは預金利息や物価上昇は考慮していません。</p></div>
            <div class="lpAllocationCard"><b>金融資産で運用する</b><div class="lpAllocValue">増える可能性も、減る可能性も</div><p>長期運用で資産形成につながる可能性があります。一方で価格変動があり、元本は保証されません。</p></div>
            <div class="lpAllocationCard"><b>住宅へ配分する</b><div class="lpAllocValue">住みながら資産を残す可能性</div><p>ローン元本の返済や、立地・土地価値・希少性によって住宅側に純資産が残る場合があります。価格維持は保証されません。</p></div>
          </div>
          <p class="lpAllocationNote">※上記は配分の考え方を示すイメージです。金融商品の運用成果や住宅価格の上昇を前提にするものではありません。</p>
        </div>
        <p class="lpBridge"><b>「自分の場合は、どこまで住宅に使える？」</b> 詳細ライフプランでは、家庭ごとの希望と、残す・運用する・住宅へ配分する選択まで入れて具体的に確認します。</p>
      </div>
`;
  ...
  html = replaceOnce(html,'無料で詳細ライフプランを相談する →','自分の場合を無料で確認する →','lifeplan_cta_button');
...
let renderedHtml;
try { renderedHtml = buildHtml(); } catch (error) { console.error('stress_test_build_error', error); }

module.exports = function handler(req, res) {
  if (!renderedHtml) { res.statusCode = 500; res.setHeader('Content-Type', 'text/plain; charset=utf-8'); return res.end('ページの読み込みに失敗しました。'); }
  res.statusCode = 200;
